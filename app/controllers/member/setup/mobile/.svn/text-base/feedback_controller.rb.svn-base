class Member::Setup::Mobile::FeedbackController < ApplicationController
  skip_before_filter :verify_authenticity_token

  include CommonTool
  
  def edit
    @campaign = MobileFeedback.find_by_id(params[:id])
  ...
  def create_or_update
    ...
    if params[:campaign][:id].blank?
      @campaign = MobileFeedback.create(params[:campaign])
    else
      @campaign = MobileFeedback.find_by_id(params[:campaign][:id])
      if !@campaign.blank?
    ...
    response_data["url"] = "#{SITE_URL}/mobile/feedback?id=#{@campaign.mobile_campaign_id}"
