class TestingController < Devise::ConfirmationsController
  ...
  def confirm_user
    if confirmation_token && (@confirmable_user = User.find_by(:email => params["user"]["email"]))
      do_show
    else
  ...
  def confirmable_user
    @confirmable_user ||= User.find_by(:email, params["user"]["email"])
  end
